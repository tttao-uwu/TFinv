...
class AttentionStore:
    ...
    def get_empty_store():
        # return {"down": [], "mid": [], "up": []}
        return {"down_cross_attn_raw": [], "mid_cross_attn_raw": [], "up_cross_attn_raw": [], 
                "down_self_attn_raw": [], "mid_self_attn_raw": [], "up_self_attn_raw": [],
                "down_cross_attn_edit": [], "mid_cross_attn_edit": [], "up_cross_attn_edit": [], 
                "down_self_attn_edit": [], "mid_self_attn_edit": [], "up_self_attn_edit": [],
                "down_cross_feat_raw": [], "mid_cross_feat_raw": [], "up_cross_feat_raw": [],
                "down_self_feat_raw": [], "mid_self_feat_raw": [], "up_self_feat_raw": [],
                "down_cross_feat_edit": [], "mid_cross_feat_edit": [], "up_cross_feat_edit": [],
                "down_self_feat_edit": [], "mid_self_feat_edit": [], "up_self_feat_edit": [],
                }

    ### NOTE: huggingface guys modify this code to only save the attention maps with 16*16
    def __call__(self, attn, is_cross: bool, place_in_unet: str, 
                        cross_replace_steps, self_replace_steps, hidden_states):
        
        shape_0dim_attn = attn.shape[0]
        shape_0dim_half_attn  = shape_0dim_attn // 2
        
        shape_0dim_feat = hidden_states.shape[0]
        shape_0dim_half_feat  = shape_0dim_feat // 2
 
        dict_key = f"{place_in_unet}_{'cross' if is_cross else 'self'}_feat_raw"
        self.step_store[dict_key].append(hidden_states[0:shape_0dim_half_feat,:,:])
        
        dict_key = f"{place_in_unet}_{'cross' if is_cross else 'self'}_attn_raw"
        if attn.shape[1] == self.attn_res**2:
            self.step_store[dict_key].append(attn[0:shape_0dim_half_attn,:,:])
            
        dict_key = f"{place_in_unet}_{'cross' if is_cross else 'self'}_feat_edit"
        self.step_store[dict_key].append(hidden_states[shape_0dim_half_feat:,:,:])
        
        dict_key = f"{place_in_unet}_{'cross' if is_cross else 'self'}_attn_edit"
        if attn.shape[1] == self.attn_res**2:
            self.step_store[dict_key].append(attn[shape_0dim_half_attn:,:,:])    
    ...
    def get_raw_attn(self, from_where: List[str], res: int, is_cross: bool) -> torch.Tensor:
        out = []
        num_pixels = res ** 2
        attention_maps = self.get_average_attention()

        for location in from_where:
            for item in attention_maps[f"{location}_{'cross' if is_cross else 'self'}_{'attn_raw'}"]:
    ...
    def get_edit_attn(self, from_where: List[str], res: int, is_cross: bool) -> torch.Tensor:
        out = []
        num_pixels = res ** 2
        attention_maps = self.get_average_attention()
 
        for location in from_where:
            for item in attention_maps[f"{location}_{'cross' if is_cross else 'self'}_{'attn_edit'}"]:
                if item.shape[1] == num_pixels:
                    cross_maps = item.reshape(-1, res, res, item.shape[-1])
                    out.append(cross_maps)
        if len(out)>0:
...
class AttentionReplace(AttentionStore):
    def __call__(self, attn, is_cross: bool, place_in_unet: str, 
        ...
        if is_cross and self.curr_step_index < cross_replace_steps:
            C, H, W = attn.shape
            attn_ = attn.reshape(2, C//2, H, W)
    
            ### NOTE: amplify
            # breakpoint()
            attn_replace=attn_[0].clone()
...
class AttentionRefine(AttentionStore):
    def __call__(self, attn, is_cross: bool, place_in_unet: str, 
        ...
        if is_cross and self.curr_step_index < cross_replace_steps:
            ...
            attn_base_replace = attn_[0][:,:,self.mapper]
            attn_replace = attn_[1] * (1-self.alphas) + attn_base_replace * self.alphas
...
class AttendExciteAttnProcessor:
    def __init__(self, attnstore, place_in_unet, cross_replace_steps=1, self_replace_steps=1):
        super().__init__()
    ...
    def __call__(
        ...
        # ### Call Attention_Store
        attention_probs = self.attnstore(attention_probs, is_cross, self.place_in_unet, 
                                         self.cross_replace_steps, self.self_replace_steps, hidden_states)
...
class StableDiffusion_EditPipeline(DiffusionPipeline, StableDiffusionMixin, TextualInversionLoaderMixin):
    r"""
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                with torch.enable_grad():
                    ...
                    # print(self_attn_0step_avg[:,:,3].sum())
        ...
        # self_attn_raw = self.attention_store.get_raw_attn(from_where=("up", "down", "mid"),
        #                                                         res=16,is_cross=False)
                                                                        
        
        # cross_attn_raw = self.attention_store.get_raw_attn(from_where=("up", "down", "mid"), 
        #                                                         res=16,is_cross=True)
        
        # self_attn_edit = self.attention_store.get_edit_attn(from_where=("up", "down", "mid"),
        #                                                         res=16,is_cross=False)
                                                                        
        
        # cross_attn_edit = self.attention_store.get_edit_attn(from_where=("up", "down", "mid"), 
        #                                                         res=16,is_cross=True)
