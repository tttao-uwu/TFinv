...
class DDIMInverseScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step(
        ...
        ### NOTE: copied from https://github.com/huggingface/diffusers/blob/main/src/diffusers/schedulers/scheduling_ddpm.py
        if model_output.shape[1] == sample.shape[1] * 2:
            model_output, _ = torch.split(model_output, sample.shape[1], dim=1)
        # else:
            # predicted_variance = None
        ### NOTE: above is to make sure the model_output is with the same shape of sample

        e_t = model_output
        x = sample
        ### ATTN: modify the time step to 999
        # prev_timestep = timestep + self.config.num_train_timesteps // self.num_inference_steps
        prev_timestep = timestep - self.config.num_train_timesteps // self.num_inference_steps
        ### NOTE: reverse ddim so exchange them 
        ### NOTE: prev_timestep > timestep
